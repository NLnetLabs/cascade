...
.. epigraph::
   ...
   The following documentation assumes Docker is already installed.
...
We download and install the NetHSM PKCS#11 driver, and create a symbolic link
to a somewhat shorter name. NetHSM documents the `PKCS#11 Setup
<https://docs.nitrokey.com/nethsm/pkcs11-setup>`_

.. code-block:: bash

    $ wget https://github.com/Nitrokey/nethsm-pkcs11/releases/download/v1.7.2/nethsm-pkcs11-v1.7.2-x86_64-linux-glibc.so
    $ sudo install nethsm-pkcs11-v1.7.2-x86_64-linux-glibc.so /usr/lib/x86_64-linux-gnu/
    $ sudo ln -sf /usr/lib/x86_64-linux-gnu/nethsm-pkcs11-v1.7.2-x86_64-linux-glibc.so $NETPKCS

We then configure the driver, in the file ``/etc/nitrokey/p11nethsm.conf``,
paying attention to the URI, usernames, and passwords.
...
these. As we haven't configured a TLS key and certificate for the device we
...
.. code-block:: bash
   ...
   $ nitropy nethsm --host $NETHSM_HOST --no-verify-tls add-user \
...
Verify NetHSM is accessible and show slots
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: bash
   ...
   # cascade template policy | tee /etc/cascade/policies/nethsm13.toml
   # sed -i -e 's|^#hsm-server-id = .\+|hsm-server-id = "kmip2pkcs11"|' /etc/cascade/policies/nethsm13.toml
...
.. code-block:: bash
   ...
   # cascade zone add --source /etc/cascade/zones/example.net --policy nethsm13 example.net
...
.. code-block:: bash
   ...
   Status report for zone 'example.net' using policy 'nethsm13'
...
.. code-block:: bash
   ...
   $ nitropy nethsm --host $NETHSM_HOST --no-verify-tls -u jj01 -p blab123456 list-keys
   Command line tool to interact with Nitrokey devices 0.11.2
...
The lowercased value `3FCCAF83FF24BDE4E0D3EE036ACAD36DEC76BD8A` is the label of the key as reported by the above tools.
