...
try:
    response_versions = requests.get(
        f"https://readthedocs.org/api/v2/version/?project__slug=cascade&active=true",
        timeout=2,
    ).json()
    versions = [
...
htmlhelp_basename = 'CascadeUserManualdoc'
...
latex_documents = [
    (master_doc, 'CascadeUserManual.tex', 'Cascade User Manual', author, 'manual'),
]
...
man_pages = [
    ...
    ('man/cascade-catalog', 'cascade-catalog', 'Manage catalog zones', author, 1),
...
texinfo_documents = [
    (master_doc, 'CascadeUserManual', 'Cascade User Manual',
     author, 'CascadeUserManual', 'One line description of project.',
     'Miscellaneous'),
]
