...
use cascade_zonedata::{
    SoaRecord, UnsignedZonePatcher, UnsignedZoneReplacer, ZoneBuilder,
    writer::{PatchError, ReplaceError},
};
use domain::{
    base::iana::Rcode,
    net::{
...
pub async fn refresh(
    ...
    if zone_builder.unsigned_curr().is_none() {
        trace!("Attempting an AXFR against {addr:?} for {:?}", zone.name);

        // Fetch the whole zone.
        axfr(zone, addr, tsig_key, zone_builder, metrics).await?;
    ...
    Ok(ixfr(zone, addr, tsig_key, zone_builder, metrics).await?)
}
...
pub async fn ixfr(
    ...
    let local_soa = zone_builder.unsigned_curr().unwrap().soa();
    ...
    match updates.next() {
        Some(Ok(ZoneUpdate::DeleteAllRecords)) => {
            // This is an AXFR.
            let mut writer = zone_builder.replace_unsigned();
            ...
            writer.set_soa(soa).map_err(AxfrError::Write)?;
            writer.apply().map_err(AxfrError::Write)?;
            return Ok(true);
        }

        Some(Ok(ZoneUpdate::BeginBatchDelete(_))) => {
            // This is an IXFR.
            let mut writer = zone_builder.patch_unsigned().unwrap();
...
fn process_ixfr(
    writer: &mut UnsignedZonePatcher,
...
fn process_axfr(
    writer: &mut UnsignedZoneReplacer,
    updates: XfrZoneUpdateIterator<'_, '_>,
    metrics: &ActiveLoadMetrics,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            IxfrError::Write(error) => {
                write!(f, "the zone could not be written: {error}")
            }
        }
    }
}
...
impl fmt::Display for AxfrError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AxfrError::Write(error) => write!(f, "could not write the zone: {error}"),
