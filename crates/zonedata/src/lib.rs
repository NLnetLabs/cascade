...
/// Whether an RRset would be generated by Cascade's signer.
///
/// `at_apex` computes whether the record is at the apex of the containing zone.
/// It is only called if necessary.
pub fn is_signing(rtype: RType, at_apex: impl FnOnce() -> bool) -> bool {
    matches!(rtype, RType::NSEC | RType::NSEC3 | RType::RRSIG)
        || (matches!(
            rtype,
            RType::NSEC3PARAM | RType::DNSKEY | RType::CDS | RType::CDNSKEY | RType::ZONEMD
        ) && (at_apex)())
}
