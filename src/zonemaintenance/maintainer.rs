...
use domain::base::{Message, MessageBuilder, Name, Rtype, Serial, ToName, Ttl};
use domain::net;
use domain::net::client::dgram::{self, Connection};
    ...
    CF: ConnectionFactory + Send + Sync + 'static,
{
    ...
    pub async fn insert_zones<T: Iterator<Item = TypedZone>>(
        ...
        // Create a deep copy of the set of zones. We will add the new zone to
        // that copied set and then replace the original set with the new set.
    ...
    CF: ConnectionFactory + 'static,
{
    ...
    async fn handle_notify(
        ...
        let (source, allow_notify_from) = match (msg.source, &zone_info.config) {
            (None, zone_cfg) => {
                ...
                Self::update_known_nameservers_for_zone(zone).await;
                Self::send_notify(zone, &zone_cfg.send_notify_to, config).await;
                return;
            }
    ...
    async fn refresh_zone_and_update_state(
        ...
        match res {
            Err(err) => {
                error!("Failed to refresh zone '{}': {err}", zone.apex_name());
    ...
    async fn refresh_zone(
        ...
        assert!(matches!(
            multi_primary_xfr_strategy,
            NotifyStrategy::NotifySourceFirstThenSequentialStoppingAtFirstNewerSerial
        ));
        ...
        let soa = read_soa(&zone.read(), zone.apex_name().clone())
            ...
            .map_err(|_out_of_zone_err| {
                ZoneMaintainerError::InternalError(
                    "Unable to read SOA for zone when checking if zone refresh is needed",
                )
            })?;
            ...
            if let Some(xfr_config) = request_xfr_from.dst(primary_addr) {
                ...
                match res {
                    ...
                    Err(err) => {
                        ...
                        warn!(
                            "Refreshing zone '{}' from {primary_addr} failed: {err}",
                            zone.apex_name()
                        );
                        saved_err = Some(err);
                    }
    ...
    async fn refresh_zone_from_addr(
        ...
        let Some(udp_client) = loaded_config
            ...
            .map_err(|err| {
                ...
                ZoneMaintainerError::ConnectionError(format!(
                    "Connecting to {primary_addr} via UDP with key '{key}' failed: {err}"
                ))
            })?
        else {
        ...
        let primary_soa_serial = Self::extract_response_soa_serial(msg).await?;
                ...
                for update in it {
                    let update = update.map_err(|_err| {
                        ZoneMaintainerError::ProcessingError(
                            domain::net::xfr::protocol::Error::Malformed,
                        )
                    })?;
                ...
                let Some(tcp_client) = loaded_config
                    ...
                    .map_err(|err| {
                        let key = key
                            ...
                            .unwrap_or("NOKEY".to_string());
                        ZoneMaintainerError::ConnectionError(format!(
                            "Connecting to {primary_addr} via TCP with key '{key}' failed: {err}"
                        ))
                    })?
                else {
                    return Err(ZoneMaintainerError::NoConnectionAvailable);
    ...
    //
    //    Default: “3 2 1 0 0”
    async fn identify_nameservers(zone: &Zone) -> Result<ZoneNameServers, ()> {
        trace!(
        ...
        let Some((soa, _)) = read_soa(&read, zone.apex_name().clone())
            ...
            error!("Unable to read SOA RRSET for zone '{}'.", zone.apex_name());
            return Err(());
        };
    ...
    async fn is_known_primary(acl: &NotifySrcDstConfig, source: &IpAddr, zone: &Zone) -> bool {
        ...
        if let Some(found_ns) = find_res {
            ...
            trace!(
                "Source IP {source} does NOT match any primary name servers for zone '{}'.",
                zone.apex_name()
            );
            false
        }
    ...
    CF: ConnectionFactory + Sync + Send + 'static,
{
    fn check_xfr_access<'a>(
        ...
        if zone_info.config().provide_xfr_to.is_empty() {
            warn!(
                "{qtype} for zone '{}' from {client_ip} refused: zone does not allow XFR",
                zone.apex_name(),
            );
    ...
    CF: ConnectionFactory + Sync + Send + 'static,
{
        ...
        Octs: Octets + Send + Sync,
    {
        ...
        let client_ip = req.client_addr().ip();
        let key_name = req.metadata().as_ref().map(|key| key.name().to_owned());
...
impl From<domain::zonetree::update::Error> for ZoneMaintainerError {
    fn from(err: domain::zonetree::update::Error) -> Self {
